...
`BookingShape` (forme cible, module Mongo pour l'instant — voir État de la
migration) :
```ts
{
...
```
⚠️ Module `bookings` pas encore migré vers Prisma : la réponse réelle
aujourd'hui expose `_id` (Mongo) au lieu de `id`, et un champ `steps[].`
sans forcément tous les champs de `BookingShape` (le mapping vers cette
forme cible arrivera avec sa propre migration). **Ne pas coder en dur un
format d'id (UUID vs ObjectId) — traiter l'id comme une string opaque.**
...
```
⚠️ Module pas migré — réponse réelle actuelle en forme Mongo (`_id`).
...
`PilgrimDocumentShape` (forme cible) :
```ts
{
...
```
⚠️ Module pas migré — forme Mongo actuelle (`_id`).
...
`RiteSheetShape` / `RiteProgressShape` (forme cible) :
```ts
...
`GroupShape` (`src/types/group.types.ts`) — **ce module vient d'être migré
sur Prisma, `id`/`packageId`/`agencyId`/`guideId` sont déjà des UUID
strings** :
```ts
{
...
`ReviewShape` (forme cible, module pas migré, forme Mongo réelle
aujourd'hui) :
```ts
...
Recommandation : traiter tout id comme une string opaque côté app (ne pas
